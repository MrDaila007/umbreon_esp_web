...
Если **`export.sh`** падает после строки про **`pkg_resources`** / **`check_python_dependencies`**, в окружении IDF не хватает **setuptools**: выполните на раннере  
`~/.espressif/python_env/rtos*_py*_env/bin/python3 -m pip install -U setuptools pip`  
(точное имя каталога см. в `ls ~/.espressif/python_env/`). Workflow перед `source` пытается сделать это автоматически.
