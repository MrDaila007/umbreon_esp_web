...
Если **`export.sh`** падает после **`pkg_resources`** / **`check_python_dependencies`**, при этом **`pip list`** уже показывает **setuptools**, откатите версию: скрипты ESP8266 RTOS SDK ожидают старый **`import pkg_resources`**; **setuptools 82** в сочетании с Python 3.12 часто даёт ту же ошибку даже при установленном пакете. На раннере:

```bash
eval "$(python3 ~/ESP8266_RTOS_SDK/tools/idf_tools.py export)"
python3 -m pip install -U 'pip' 'setuptools>=69,<82'
source ~/ESP8266_RTOS_SDK/export.sh
```

В CI см. **Build firmware** в `.github/workflows/build.yml`.
